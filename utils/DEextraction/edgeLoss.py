...
def canny(image):
    ...
    edge = cv2.Canny(img, 50, 125)
    # todo change data format
    edge = torch.tensor(edge)
    edge = torch.autograd.Variable(edge).to(device)
    return edge
...
class EdgeSaliencyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_gt):
        ...
        sal_loss = self.weighted_bce(input_=y_pred, target=y_gt, weight_0=1.0, weight_1=1.12)  # todo change to CrossEntropyLoss
...
if __name__ == '__main__':
    ...
    image = '../../doc/2-input/1af8c6178ae6b17c7f37b0beff0ee791.jpg'
    edge = canny(image, size=(2, 1, 8, 16))
